...
let app;try{
 ...
 await p.getByRole('button',{name:'Start automatic',exact:true}).click();await p.locator('#ask-yes').click();await p.getByRole('button',{name:'Pause automatic',exact:true}).waitFor();await p.getByRole('button',{name:'Pause automatic',exact:true}).click();assert.equal(await fs.readFile(path.join(watch,'notes.txt'),'utf8'),'keep me');
