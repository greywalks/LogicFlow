...
(async()=>{
 const browser=await _electron.launch({executablePath:path.resolve('node_modules/electron/dist/electron'),args:['--no-sandbox',path.resolve('qa/browser-host.cjs')],env:{...process.env,DISPLAY:process.env.DISPLAY||'127.0.0.1:97'}});
 ...
 let state={defaults:c,config:null,needsSetup:true,running:false,moved:0,lastCheck:null,lastError:'',version:'3.2.0'},callback=()=>{};
 window.__calls=[];
 ...
 await page.getByRole('button',{name:'Start automatic',exact:true}).click();await page.locator('#ask-yes').click();await page.getByRole('button',{name:'Pause automatic',exact:true}).click();
